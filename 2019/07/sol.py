...
for li, line in enumerate(lines):
    ...
    phases = [0, 1, 2, 3, 4]
    best = 0
    for pperm in it.permutations(phases):
        i_sig = 0
        i_sig = Intcode(inss, (pperm[0], i_sig)).exec()
        i_sig = Intcode(inss, (pperm[1], i_sig)).exec()
        i_sig = Intcode(inss, (pperm[2], i_sig)).exec()
        i_sig = Intcode(inss, (pperm[3], i_sig)).exec()
        final = Intcode(inss, (pperm[4], i_sig)).exec()
        if final > best:
            best = final
    print(best)
